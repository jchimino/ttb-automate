...
1. ANTHROPIC — ANTHROPIC_API_KEY is set.
               OCR text + images sent to claude-haiku-4-5.
                              Takes precedence over local models when API key is present.

                              2. VISION    — llava:7b available in Ollama (GPU present) and no API key.
                                             llava reads the image directly; Tesseract OCR runs in
                                                            ...
                                                            3. RECONCILE — No GPU, no API key.
                                                                           ...
                                                                           Set ANTHROPIC_API_KEY in .env to use the Anthropic API as the primary model.
                                                                           Local inference (strategy 2 or 3) is the fallback when no API key is set.
                                                                           """
import base64
import io
import json
...
async def call_anthropic(prompt: str) -> str:
      """POST to Anthropic Messages API. Primary option when API key is set."""
    async with httpx.AsyncClient(timeout=60.0) as client:
              r = await client.post(
...
async def run_vision(label_bytes, form_bytes, submission_id):
      """
          Vision path: llava:7b reads the label image directly.
              Tesseract OCR runs in parallel and its output is appended as
                  supplementary confirmation text.  llava's visual interpretation
                      takes precedence.
                          """
...
async def run_reconcile(label_bytes, form_bytes, submission_id):
      """
          Reconcile / fallback path: Tesseract OCR extracts text, then either
              Anthropic (if API key set — primary) or local qwen2.5:7b evaluates
                  compliance.
                      """
    all_images = ([form_bytes] if form_bytes else []) + label_bytes
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=500,
                                                                detail=f"Assessment failed: {type(exc2).__name__}: {exc2}") from exc2
