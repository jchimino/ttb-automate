...
Compliance Prompts
==================
Two prompt builders — one for the vision path (llava:7b, GPU), one for the
OCR/reconcile path (qwen2.5:7b, CPU).

IMPORTANT: Both builders use identical CFR rules and scoring logic.
The government health warning is a hard binary PASS/FAIL — partial presence
is never sufficient. post_process() in models.py enforces these rules
deterministically after the LLM responds.
"""
          ...
          5. bottler_info   — producer name AND city/state or country
            ...
            Wine (27 CFR Part 4):
              VALID designations: Table Wine, Red Wine, White Wine, Rosé Wine, Pink Wine,
                  Red Table Wine, White Table Wine, Rosé Table Wine, Dessert Wine,
                      Sparkling Wine, Champagne, Port, Sherry, Merlot, Cabernet Sauvignon,
                          Chardonnay, Pinot Noir (varietal = class if ≥75% of stated grape)
                            NOTE: "Red Wine", "White Wine", "Rosé Wine", and "Pink Wine" ARE valid type
                                designations under 27 CFR 4.21(a)(1) — they describe a still grape wine
                                    by color. A label showing "Red Wine" satisfies the class/type requirement.
...
PASS: Both clauses are FULLY and COMPLETELY readable in the label image.
  Both the pregnancy/birth defects clause AND the impairment/machinery clause
    ...
    FAIL: Either clause is absent, truncated, rotated to the point of being
      unreadable, obscured, or cannot be confirmed as complete.
              ...
              REVIEW  — a field is genuinely ambiguous (partially visible, conflicting data);
                         ...
                         DENY    — a mandatory field is absent, incomplete, or uses an invalid
                                    ...
                                    PASS   — field is present and meets the TTB Standard of Identity
                                              ...
                                              REVIEW — field is genuinely ambiguous (conflicting data, genuinely partially
                                                        visible — not just rotated or small print)
                                                        FAIL   — field is absent, uses an invalid designation, or (for health_warning)
                                                                  either statutory clause is missing
...
def build_prompt_vision(
        ...
        """
            Primary path — llava:7b reads the label images directly (GPU).
    ...
    if has_form:
            ...
            prompt += (
                            f"\nYou are shown {n_labels} label image(s). "
...
def build_prompt_ocr(
        ...
        """
            Fallback path — OCR text is the primary input (no GPU / llava unavailable).
                Uses identical CFR rules as the vision path for consistent decisions.
                    """
        prompt  = "You are a TTB (Alcohol and Tobacco Tax and Trade Bureau) compliance specialist.\n\n"
        prompt += _TTB_RULES

    prompt += f"""
    The following text was extracted from the submission images by OCR (four
    rotation passes were run to capture sideways text like the government warning).
    ...
    if has_form:
                prompt += (
                                ...
                                f"values. The remaining {n_labels} image(s)/block(s) are the label(s)."
                )
else:
            prompt += "\nNo application form was provided. Assess against TTB requirements only."
