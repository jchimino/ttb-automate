...
Backend engine is selected at request time:
  LOCAL_LLM_URL set   →  local Ollama service (assess:8000)
...
class VerifyResponse(BaseModel):
    ...
    cloud_api: bool = False               # True when Anthropic handled inference
    cloud_provider: Optional[str] = None  # "Anthropic" when cloud_api is True
...
async def _verify_with_local_llm(image_b64: str) -> VerifyResponse:
      """Call the local assess service and map the result."""
    import httpx
    try:
        ...
        raise HTTPException(
                      status_code=502,
                      detail=(
                                        "AI model is still loading (~4.4 GB on first boot). "
                                        "The UI will retry automatically. "
                                        "See the README → Setup section for details."
                      ),
        ) from exc
except httpx.HTTPStatusError as exc:
        raise HTTPException(
                      status_code=502,
                      detail=f"Assessment service returned HTTP {exc.response.status_code}: {exc.response.text[:200]}",
        ) from exc
except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Local assessment failed: {exc}") from exc
...
async def _verify_with_anthropic(
      ...
      """Two-step Claude flow: classify commodity → BAM compliance verify."""
    try:
    ...
    if not ANTHROPIC_API_KEY:
              raise HTTPException(
                            status_code=500,
                            detail=(
                                              "No ANTHROPIC_API_KEY configured and LOCAL_LLM_URL is not set. "
                                              "Set ANTHROPIC_API_KEY in .env to use cloud inference."
                            ),
              )
